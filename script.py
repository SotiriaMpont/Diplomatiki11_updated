...
def Apodoxi_Aitimatos_Distributor():
    ...
    for aitima in aitimata:
        while True:
            ...
            # τυχαία αποφασίζω αν θα αναλάβει ή θα απορρίψει το αίτημα 
            ...
            if value == 1:
                ...
                # If the request was declined, assign it to another distributor
    ...
    if len(result1) == 0:
        ...
        for request in result1:
            ...
            if len(result2) == 0:
                ...
                rating = str(random.randint(1, 5)) # random integer between 1 and 5
                ...
                if len(result3) > 0:
                    ...
                    val4 = (distributor_id, current_date, request[0], rating)

                    try:
                        cursor.execute(sql4, val4)
                        mydatabase.commit()
                        print("Rating of", rating, "inserted for distributor with id:", distributor_id, "and request with id:", request[0])
...
def Rating_From_Company():
    ...
    for request in requests:
        ...
        # Check if rating already exists
        sql3 = "SELECT * FROM RatingFromCompany WHERE id_di=%s AND dat_shift=%s AND id_aitim=%s"
        val3 = (distributor_id, current_date, request[0])
        cursor.execute(sql3, val3)
        result3 = cursor.fetchall()
        ...
        if len(result3) > 0:
            ...
            sql4 = "INSERT INTO RatingFromCompany (id_di, dat_shift, id_aitim, Rating_company) VALUES (%s, %s, %s, %s)"
            val4 = (distributor_id, current_date, request[0], rating)

            try:
                cursor.execute(sql4, val4)
                mydatabase.commit()
                print(f"Rating of {rating} for distributor ID {distributor_id}, delivery date {current_date}, and order ID {request[0]} has been stored.")
            except mysql.connector.Error as error:
                print(f"Failed to insert record into RatingFromCompany table: {error}")
    ...
    for result1 in results1:
        ...
        if len(result2) == 0:
            ...
            if len(result3) != 0:
                ...
                cursor = mydatabase.cursor()
                sql4 = "INSERT INTO RatingFromCostumer (dat_shif_costumer, id_aitimatos_costumer, id_rating_costumer , criterion1, criterion2, criterion3) VALUES (%s, %s, %s, %s, %s, %s)"
...
def metrics():
    ...
    for distributor_id in distributor_ids:
        ...
        for shift in shifts:
            shift_start_time = shift[8]
            shift_end_time = shift[9]
            shift_date = shift[0]
        ...
        # μο ανα εκτιμώμενων χλμ / accepted αιτήματα
            ...
            # Perform the overhead calculation
            query = """
                SELECT 
                    s.date_shift, 
                    s.ID_distributor_shift, 
        ...
        #gia na mpoun oi times sto metrics_costumer                   
            query_rating_costumer1 = """
                SELECT 
                    ...
                    AVG(r.criterion1) as criterion1,
                    AVG(r.criterion2) as criterion2,
                    AVG(r.criterion3) as criterion3
            ...
            for row5 in results_rating_costumer1:        
              cursor.execute("INSERT IGNORE INTO metrics_costumer (criterion1, criterion2, criterion3, distributor_id, shift) VALUES (%s, %s, %s, %s, %s)",
            ...
            query_rating_store1 = """
                    SELECT 
                        ...
                        r.id_di, 
                        r.id_aitim,
                        AVG(r.Rating_store) as Rating_store
            ...
            for row12 in results_rating_store1:        
                cursor.execute("INSERT IGNORE  INTO metrics_store ( Rating_stores,distributor_id, shift ) VALUES (%s,%s,%s)",
                 (row12[3],row12[1],row12[0]))
                            
            query_rating_company = """
                    SELECT 
                        r.dat_shift, 
                        r.id_di, 
                        r.id_aitim,
                    AVG( r.Rating_company) as Rating_company 
            ...
            cursor.execute(query_rating_company, (distributor_id,))
            results_rating_company = cursor.fetchall()
            
           

            for row12 in results_rating_company:        
                cursor.execute("INSERT IGNORE INTO metrics_company ( Rate_Company,distributor_id, shift) VALUES (%s,%s,%s)   ",
                 (row12[3],row12[1],row12[0]))
